...
def spHRRpTrhou(save_dir, ev_num=1):
    ...
    for i, df in data_nout_dict.items():
        ...
        for i_sp, sp_name in enumerate(sp_list):
            df.plot(kind='line', ax=ax[0], x='x_m', y=sp_name, lw=lw, marker=mk, c=ctab[i_sp], legend=False, label=sp_name)
        df.plot(kind='line', ax=ax_0t, x='x_m', y='HRR', lw=lw, marker=mk, c='firebrick', legend=False, label='HRR')
        df.plot(kind='line', ax=ax[1], x='x_m', y='  T', lw=lw, marker=mk, c=ctab[0],     legend=False, label='T')
        df.plot(kind='line', ax=ax_1t, x='x_m', y='  p', lw=lw, marker=mk, c=ctab[1],     legend=False, label='p')
        df.plot(kind='line', ax=ax[2], x='x_m', y='u_x', lw=lw, marker=mk, c=ctab[2],     legend=False, label='u')
        df.plot(kind='line', ax=ax_2t, x='x_m', y='rho', lw=lw, marker=mk, c=ctab[3],     legend=False, label='rho')
