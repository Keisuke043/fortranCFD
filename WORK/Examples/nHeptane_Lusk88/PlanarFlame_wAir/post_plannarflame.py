...
def spHRRpT(save_dir, ev_num=1):

    for i, df in data_nout_dict.items():

        fig, ax = plt.subplots(nrows=2,ncols=1, sharex='col', figsize=(12,9))
        ax_1t = ax[1].twinx()
        ...
        df.plot(kind='line', ax=ax[1], x='xcm', y='HRR', lw=lw, marker=mk, c='firebrick', legend=False, label='HRR')
        df.plot(kind='line', ax=ax_1t, x='xcm', y='  T', lw=lw, marker=mk, c=ctab[0],     legend=False, label='T')

        ax[0].set_ylabel('Mole fraction [-]')
        ax[1].set_ylabel('Heat release rate [J/m$^3$-s]')
        ax_1t.set_ylabel('T [K]')
        ax[1].set_xlabel('x [cm]')

        ax[0].set_ylim(sp_lim)
        ax[1].set_ylim(HRRlim)
        ax_1t.set_ylim(Tlim)
        ax[1].set_xlim(xlim)
...
if __name__ == "__main__":
    ...
    ### setting params
    read_dir_name = './RESULTS_planar_flame_hdf'
    read_dir_name = './RESULTS_planar_flame_t0_40_hdf'
    read_dir_name = './RESULTS_planar_flame_igtest_hdf'
    read_dir_name = './RESULTS_planar_flame_igtest_t0_40_hdf'
    read_dir_name = './RESULTS_planar_flame*_hdf'
    ...
    ulim = [-50, 150] # m/s
    rholim = [0, 3.2] # kg/m^3
    HRRlim = [1e+2, 1.5e+9] # J/m^3-s
    sp_lim = [0, 0.22]

    HRRlim_log = [1e+3, 2e+10]
    sp_lim_log = [1e-9, 9e-1]

    sp_list = ['nc7h16', 'o2', 'ch2o', 'co', 'co2', 'h2o']

    read_dirs = glob.glob(read_dir_name)
    print(read_dirs)

    for read_dir_name in read_dirs:
        ...
        save_dir = './{0}/{1}_spHRRpT_png'.format(save_dir_base, dir_name)
        if flag_log:
        ...
        for i in num_list:
            ...
            spHRRpT(save_dir, ev_num=every_num_graph)
        mkmovie(save_dir)
