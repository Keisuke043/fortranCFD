...
def spHRRpTrhou(save_dir, ev_num=1):
    ...
    for i, df in data_nout_dict.items():
        ...
        fig, ax = plt.subplots(nrows=3,ncols=1, sharex='col', figsize=(12,14))
        ax_0t = ax[0].twinx()
        ax_1t = ax[1].twinx()
        ax_2t = ax[2].twinx()
        ...
        for i_sp, sp_name in enumerate(sp_list):
            df.plot(kind='line', ax=ax[0], x='xcm', y=sp_name, lw=lw, marker=mk, c=cm.colors[i_sp], legend=False, label=sp_name)
        df.plot(kind='line', ax=ax_0t, x='xcm', y='HRR', lw=lw, marker=mk, c='firebrick', legend=False, label='HRR')
        df.plot(kind='line', ax=ax[1], x='xcm', y='  T', lw=lw, marker=mk, c=ctab[0],     legend=False, label='T')
        df.plot(kind='line', ax=ax_1t, x='xcm', y='  p', lw=lw, marker=mk, c=ctab[1],     legend=False, label='p')
        df.plot(kind='line', ax=ax[2], x='xcm', y='u_x', lw=lw, marker=mk, c=ctab[2],     legend=False, label='u')
        df.plot(kind='line', ax=ax_2t, x='xcm', y='rho', lw=lw, marker=mk, c=ctab[3],     legend=False, label='rho')
        ...
        ax[0].legend(handler_0o+handler_0t, label_0o+label_0t, loc='upper left', bbox_to_anchor=(0.0, 1.25,), 
                                                                        borderaxespad=0, ncol=7, fontsize=11)
...
if __name__ == "__main__":
    ...
    ### setting params
    read_dir_name = 'RESULTS_shock_hdf'
    flag_log = True
    flag_marker = False
    every_num_graph = 1
    ...
    xlim = None # cm
    Tlim = [1500, 4000] # K
    plim = [2.9e+5, 5.1e+6] # atm
    ulim = [-290, 520] # m/s
    rholim = [0.0, 8.2] # kg/m^3
    HRRlim = [0.0, 1e+8] # J/m^3-s
    sp_lim = [0.0, 0.22]

    HRRlim_log = [1e+3, 2e+10]
    sp_lim_log = [1e-14, 1.0]

    sp_list = ['N-C7H16', 'O2', 'N2', 'H2', 'CH2O', 'H2O2', 'OH', 'HO2', 'CO', 'CO2', \
               'C2H2', 'C2H4', 'C2H6', 'C3H3', 'C3H6', 'A1-C6H6', 'A1OH', 'A1CH3', 'A1C2H5']
