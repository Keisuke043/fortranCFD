!  
! This Ckinterp_PLOG.f is to replace ckinterp.f in Chemkin II and III for PLOG functions used in unamolecular reactions
!
! This subroutine is made only for personal use of combustion research and should not be distributed to a third party. For more detailed
! information, Users are adviced to read the reference below to use this subroutine.
!
! 
!---------------------------------------------------------------------------------------------------------------
!Reference: X. Gou, J.A. Miller, W. Sun and Y. Ju, 
!		"Implementation of PLOG function in Chemkin II and III", http://engine.princeton.edu, 2011.
!
!--------------------------------------------------------------------------------------------
C@(#)===================================================================
C@(#)                   FILE =  ckinterp.f
...
C
      PROGRAM CKINTP
C    
C     CKINTERP is the gas-phase, symbolic, chemical mechanism pre-
...
!      PARAMETER (MDIM=20,KDIM=280,MKDIM=MDIM*KDIM,IDIM=5000,LSYM=16,
...
!     7           NOIDIM=MAXORD*IDIM)
      PARAMETER (LSYM=16,NPAR=3,NPC=5,NPCP2=NPC+2,MAXTP=3,
     &           NTR=MAXTP-1,MAXSP=8,MAXTB=20,NLAR=2,
     &           NFAR=8,LIN=15,LOUT=16,LTHRM=17,LINC=25,CKMIN=1.0E-3)
...
!     2          UPCASE*4, VERS*(LSYM), PREC*(LSYM)
	...
	PARAMETER(MAXNPPLOG=100)!For PLOG
      
      CHARACTER*(LSYM),ALLOCATABLE::KNAME(:),ENAME(:);
	CHARACTER SUB(80)*80,LINE*80,IUNITS*80,AUNITS*4,EUNITS*4,
     &          UPCASE*4,VERS*(LSYM),PREC*(LSYM);

	CHARACTER KEY(6)*4; !KEY(5)*4, Add one for PLOG
     	CHARACTER*80 PLOGSTR; !FOR PLOG
	INTEGER*2 NARG;  !Added for arg @20101117
	CHARACTER*100 ARGSTR1,ARGSTR2; !Added for arg @20101117,#1:Inp File, #2:Bin File. 

	!--PLOG議膨倖歌方才泣方
	REAL*8,ALLOCATABLE::P_PLOG(:,:),A_PLOG(:,:),N_PLOG(:,:),
     &                    E_PLOG(:,:);
	INTEGER,ALLOCATABLE::NP_PLOG(:),NM_PLOG(:);
...
!     9          KORD(MAXORD,IDIM), RORD(MAXORD,IDIM)
      REAL*8,ALLOCATABLE::AWT(:),WTM(:),A(:,:,:), T(:,:),PAR(:,:),
     &          RPAR(:,:),PLAN(:,:),RLAN(:,:),WL(:),PFAL(:,:),
     &          AIK(:,:),RNU(:,:),RORD(:,:);
      INTEGER,ALLOCATABLE::KNCF(:,:),KPHSE(:),KCHRG(:),NT(:),NSPEC(:),
     &          NREAC(:),NU(:,:),NUNK(:,:),IDUP(:),IREV(:),ILAN(:),
     &          IRLT(:),IWL(:),IFAL(:), IFOP(:), KFAL(:),ITHB(:),
     &          NTBS(:),NKTB(:,:),IRNU(:),IORD(:),KORD(:,:);

      DIMENSION VALUE(5);
      LOGICAL KERR,THERMO;
	LOGICAL,ALLOCATABLE::ITHRM(:);

      PARAMETER (NJAR=9,NF1R=4);
 ...
 !    &          IFT1(IDIM), PFT1(NF1R,IDIM), IEXC(IDIM), PEXC(IDIM)
      ...
      INTEGER,ALLOCATABLE::IEIM(:),ITDEP(:),IJAN(:),IFT1(:),IEXC(:);
      REAL*8,ALLOCATABLE::PJAN(:,:),PFT1(:,:),PEXC(:);

      !---資函塰佩歌方佚連
	NARG=IARGC();
	IF(NARG>=2)THEN
	  CALL GetARG(1,ARGSTR1); !Inp 猟周兆
	  CALL GetARG(2,ARGSTR2); !Bin 猟周兆
	END IF

      !==資函字尖議児云佚連
	CALL GETSPACE_GOU(MM,KK,II);!WRITE(*,*) MM,KK,II;

      !---柴麻斤哘械方
        MDIM=MM;KDIM=KK;MKDIM=MDIM*KDIM;IDIM=II;NPIDIM=IDIM*NPAR;
        NKTDIM=NTR*NPCP2*KDIM;NSIDIM=MAXSP*IDIM;NTIDIM=MAXTB*IDIM;
        NLIDIM=NLAR*IDIM;NFIDIM=NFAR*IDIM;
        NTDIM=KDIM*MAXTP;NIDIM=11*IDIM;MAXORD=KDIM;NOIDIM=MAXORD*IDIM;
        NJIDIM=NJAR*IDIM;NF1IDIM=NF1R*IDIM;
      !---腎寂蛍塘
	ALLOCATE(KNAME(KDIM),ENAME(MDIM));
      ALLOCATE(AWT(MDIM), KNCF(MDIM,KDIM), WTM(KDIM), KPHSE(KDIM),
     1         KCHRG(KDIM), A(NPCP2,NTR,KDIM), T(MAXTP,KDIM), NT(KDIM),
     2         NSPEC(IDIM), NREAC(IDIM), NU(MAXSP,IDIM),
     3         NUNK(MAXSP,IDIM), PAR(NPAR,IDIM), IDUP(IDIM),IREV(IDIM),
     4         RPAR(NPAR,IDIM), ILAN(IDIM), PLAN(NLAR,IDIM),
     ...
     9         KORD(MAXORD,IDIM), RORD(MAXORD,IDIM));
      ...
      ALLOCATE(IEIM(IDIM),ITDEP(IDIM),IJAN(IDIM),PJAN(NJAR,IDIM),
     &         IFT1(IDIM),PFT1(NF1R,IDIM),IEXC(IDIM),
     &         PEXC(IDIM),ITHRM(KDIM));

	!--PLOG議膨倖歌方才泣方
	ALLOCATE(P_PLOG(IDIM,MAXNPPLOG),A_PLOG(IDIM,MAXNPPLOG),
     &  N_PLOG(IDIM,MAXNPPLOG),E_PLOG(IDIM,MAXNPPLOG),NP_PLOG(IDIM),
     &  NM_PLOG(IDIM));
...
!     &     PFT1/NF1IDIM*0.0/, PEXC/IDIM*0.0/
      ...
      DATA NEIM,NJAN,NFT1,NEXC/4*0/;

           IEIM(1:IDIM)=0;ITDEP(1:IDIM)=0;
           IJAN(1:IDIM)=0;IFT1(1:IDIM)=0;PJAN(1:NJAR,1:IDIM)=0.0;
           PFT1(1:NF1R,1:IDIM)=0.0;PEXC(1:IDIM)=0.0;

      !--Initialize variables
      DATA KEY/'ELEM','SPEC','THER','REAC','END','PLOG'/, KERR/.FALSE./,
     &     ITASK,NCHRG,MM,KK,II,NLAN,NFAL,NTHB,NREV,NRLT,NWL,
     &     NRNU,NORD/13*0/,THERMO/.TRUE./;

	     ENAME(1:MDIM)=' ';AWT(1:MDIM)=0.0;
	     T(1:MAXTP,1:KDIM)=-1.0;
           KNAME(1:KDIM)=' ';WTM(1:KDIM)=0.0;NT(1:KDIM)=3;
           KPHSE(1:KDIM)=0;KCHRG(1:KDIM)=0;ITHRM(1:KDIM)=.FALSE.;
           WL(1:IDIM)=0.0;IFOP(1:IDIM)=-1;NTBS(1:IDIM)=0;IDUP(1:IDIM)=0;

           NSPEC(1:IDIM)=0;NREAC(1:IDIM)=0;IREV(1:IDIM)=0;

		 ILAN(1:IDIM)=0;IRLT(1:IDIM)=0;IWL(1:IDIM)=0;
		 IFAL(1:IDIM)=0;KFAL(1:IDIM)=0;ITHB(1:IDIM)=0;
		 IRNU(1:IDIM)=0;IORD(1:IDIM)=0;

           NUNK(1:MAXSP,1:IDIM)=0;NU(1:MAXSP,1:IDIM)=0;
           NKTB(1:MAXTB,1:IDIM)=0;AIK(1:MAXTB,1:IDIM)=-1.0;

	     RNU(1:MAXSP,1:IDIM)=0.0;
		 KORD(1:MAXORD,1:IDIM)=0;RORD(1:MAXORD,1:IDIM)=0;
           PAR(1:NPAR,1:IDIM)=0.0;RPAR(1:NPAR,1:IDIM)=0.0;
	     PLAN(1:NLAR,1:IDIM)=0.0;RLAN(1:NLAR,1:IDIM)=0.0;
           PFAL(1:NFAR,1:IDIM)=0.0;KNCF(1:MDIM,1:KDIM)=0.0;
	     A(1:NPCP2,1:NTR,1:KDIM)=0.0;

		 !--PLOG data intialize
		 NP_PLOG(1:IDIM)=0;
  	     P_PLOG(1:IDIM,1:MAXNPPLOG)=0.0;
  	     A_PLOG(1:IDIM,1:MAXNPPLOG)=0.0;
  	     N_PLOG(1:IDIM,1:MAXNPPLOG)=0.0;
  	     E_PLOG(1:IDIM,1:MAXNPPLOG)=0.0;
	!=================================================
      MM=0;KK=0;II=0; !嶷仟鹸了  ，，駅倬咀葎MM/KK/II勣序佩拙紗

	
	OPEN(LOUT,FORM='FORMATTED',STATUS='UNKNOWN',FILE='chem.out');

      !--ADD BY GXL 20080205
      !-- GETDAT and GETTIM supported by ifort only !(FW 20130619)
      !CALL GETDAT(IYR,IMON,IDAY);
      !CALL GETTIM(IHR,IMIN,ISEC,I100TH);
      WRITE (LOUT,*)'!-------------------------------------------'
      WRITE (LOUT,'(A2,10X,I2,A1,I2,A1,I4,A4,I2,A1,I2,A1,I2)') 
     &       ' !',IMON,'-',IDAY,'-',IYR,' ',IHR,':',IMIN,':',ISEC
      WRITE (LOUT,*)'!             Created By KDOC 2.0'
      WRITE (LOUT,*)'!-------------------------------------------'
...
C*****END precision > single
	...
	IF(NARG>=2)THEN
      OPEN(LIN,FORM='FORMATTED',STATUS='OLD',FILE=ARGSTR1,ERR=11111);
	ELSE
      OPEN(LIN,FORM='FORMATTED',STATUS='OLD',FILE='chem.inp',ERR=11111);
	END IF

      READ(LIN,'(A)',END=11111);REWIND(LIN);

  100 CONTINUE
      LINE=' ';READ(LIN,'(A)',END=5000)LINE;

  105 CONTINUE
      !LINE='PLOG /0.0263 1.44E63 -15.66 24018./ ';
      ILEN=IPPLEN(LINE);                 !資函嗤丼海業
	IF(ILEN.EQ.0) GOTO 100;

      CALL CKISUB(LINE(:ILEN),SUB,NSUB); !函誼光倖何蛍佚連

      !-IS THERE A KEYWORD?
      !CALL CKCOMP(UPCASE(SUB(1),4),KEY,5,NKEY);
      CALL CKCOMP(UPCASE(SUB(1),4),KEY,6,NKEY);
      IF(NKEY.GT.0)ITASK=0;
      !ITASK=NKEY;
	!...................
      IF(NKEY==6)THEN !PLOG
	  ITASK=NKEY;PLOGSTR='';
	  IS1=INDEX(LINE,'/');IE1=ILASCH(LINE);
	  
	  PLOGSTR(1:IE1-IS1-1)=LINE(IS1+1:IE1-1);
        CALL IPPARR(PLOGSTR,-1,4,VALUE,NVAL,IER,LOUT);
	  IF(IER<=0)THEN
	    IF(II>0)THEN
	      NP_PLOG(II)=NP_PLOG(II)+1;
  	      !P_PLOG(II,NP_PLOG)=VALUE(1);A_PLOG(II,NP_PLOG)=VALUE(2);
  	      !N_PLOG(II,NP_PLOG)=VALUE(3);E_PLOG(II,NP_PLOG)=VALUE(4);
  	      P_PLOG(II,NP_PLOG(II))=VALUE(1); !```
		  A_PLOG(II,NP_PLOG(II))=VALUE(2);
  	      N_PLOG(II,NP_PLOG(II))=VALUE(3);
		  E_PLOG(II,NP_PLOG(II))=VALUE(4);
	     !Rate coefficients at P=1.00E-01(atm)          1.77E+30   -7.5     4332.0
!@20110123		 WRITE(*,9999)
!     &           'Rate coefficients at P=',VALUE(1),'(atm)',
!     &           VALUE(2),VALUE(3),VALUE(4)
	      !Stop;
	      WRITE(LOUT,9999)
     &           'Rate coefficients at P=',VALUE(1),'(atm)',
     &           VALUE(2),VALUE(3),VALUE(4)
           !9999 FORMAT ('  ',A,T53,1PE8.2,A,2X,1PE8.2,2X,1PE8.2)
9999      FORMAT (6X,A,1PE10.3,A,2X,1PE10.3,2X,1PE10.3,2X,1PE10.3)

		END IF
	    ITASK=4; !Added @20101026
	    GOTO 100;
	  END IF
	ELSE IF(NKEY.EQ.1.OR.NKEY.EQ.2)THEN    !ELEMENT OR SPECIES DATA
        ITASK=NKEY;
        IF(NSUB.EQ.1)GOTO 100;

        DO N=2,NSUB
          SUB(N-1)=' ';SUB(N-1)=SUB(N);
        END DO

         NSUB=NSUB-1;
      ELSEIF(NKEY.EQ.3)THEN  !THERMODYNAMIC DATA
        IF(NSUB.GT.1) THEN
  ...
  311     CONTINUE
     ...
     2                 KCHRG,NT,T,TLO,TMID,THI,KNCF,ITHRM,LOUT,KERR);

          I1=IFIRCH(LINE);
          IF(UPCASE(LINE(I1:),4).EQ.'REAC') GOTO 105
        ELSEIF(NKEY.EQ.4)THEN
          ITASK=4;
          !START OF REACTIONS; ARE UNITS SPECIFIED?
          CALL CKUNIT(LINE(:ILEN),AUNITS,EUNITS,IUNITS);
          
		IF(THERMO)THEN !THERMODYNAMIC DATA
            OPEN(LTHRM,FORM='FORMATTED',STATUS='OLD', 
     &                  FILE='therm.dat',ERR=22222);
  312       CONTINUE
            READ(LTHRM,'(A)',END=22222)LINE;
            IF(IPPLEN(LINE).LE.0.OR.INDEX(LINE,'THERM').GT.0
     &         .OR.INDEX(LINE,'therm').GT.0) GOTO 312;
     ...
     &                  KCHRG,NT,T,TLO,TMID,THI,KNCF,ITHRM,LOUT,KERR);
      ...
      !====功象ITSAK序佩斤哘侃尖   
      IF(ITASK.EQ.1)THEN !ELEMENT DATA
         IF(MM.EQ.0)THEN
            WRITE(LOUT, 200);WRITE(LOUT, 300);WRITE(LOUT, 200);
         ENDIF

         IF(NSUB.GT.0)THEN
           M1=MM+1;
           CALL CKCHAR(SUB,NSUB,MDIM,ENAME,AWT,MM,KERR,LOUT); !資函欺Element方楚才斤哘議圷殆
		 ...
		 DO M=M1,MM
             ...
             IF(AWT(M).LE.0)THEN; 
		     KERR=.TRUE.;WRITE(LOUT,1000) ENAME(M);
             ENDIF
           END DO
         
	   ENDIF
      ELSEIF(ITASK.EQ.2)THEN !PROCESS SPECIES DATA
        IF(KK.EQ.0) WRITE(LOUT,200);
        IF(NSUB.GT.0) CALL CKCHAR(SUB,NSUB,KDIM,KNAME,WTM,KK,KERR,LOUT);!SPECIES 

      ELSEIF (ITASK.EQ.4) THEN !PROCESS REACTION DATA
        IND=0;
        DO N=1,NSUB
	  ...
	  IF(IND.GT.0) THEN !AUXILIARY REACTION DATA
     ...
     7                NFT1,IFT1,NF1R,PFT1,NEXC,IEXC,PEXC)
        ...
        ELSE !THIS IS A REACTION STRING
           IF(II.LT.IDIM)THEN
             IF(II.GT.0) !CHECK PREVIOUS REACTION FOR COMPLETENESS
 ...
 5000 CONTINUE
  ...
  313    CONTINUE
         ...
         READ(LTHRM,'(A)',END=22222)LINE;
         IF(IPPLEN(LINE).LE.0.OR.INDEX(LINE,'THERM').GT.0
     &      .OR.INDEX(LINE,'therm').GT.0) GOTO 313
     ...
     &               KCHRG,NT,T,TLO,TMID,THI,KNCF,ITHRM,LOUT,KERR);
      ...
      !--OPEN LINKING FILE
      IF(NARG>=2)THEN
	  OPEN(LINC,FORM='UNFORMATTED',STATUS='UNKNOWN',FILE=ARGSTR2);
	ELSE
        OPEN(LINC,FORM='UNFORMATTED',STATUS='UNKNOWN',FILE='chem.bin');
      END IF
	...
	WRITE(LINC) VERS,PREC,KERR;
     ...
     &        +NORD*(1+MAXORD)+2*NEIM+NJAN+NFT1+NEXC;
     ...
     &       (NU(M,I),NUNK(M,I),M=1,MAXSP),I=1,II);
        ...
        IF(NREV.GT.0)WRITE(LINC)(IREV(N),(RPAR(L,N),L=1,NPAR),N=1,NREV);
     ...
     &    (ITHB(N),NTBS(N),(NKTB(M,N),AIK(M,N),M=1,MAXTB),N=1,NTHB);
        ...
        IF(NLAN.GT.0)WRITE(LINC)(ILAN(N),(PLAN(L,N),L=1,NLAR),N=1,NLAN);

        IF(NRLT.GT.0)WRITE(LINC)(IRLT(N),(RLAN(L,N),L=1,NLAR),N=1,NRLT);

        IF(NWL.GT.0)WRITE(LINC)(IWL(N),WL(N),N=1,NWL);

        IF(NEIM.GT.0)WRITE(LINC)(IEIM(N),ITDEP(N),N=1,NEIM);

        IF(NJAN.GT.0)WRITE(LINC)(IJAN(N),(PJAN(L,N),L=1,NJAR),N=1,NJAN);

        IF(NFT1.GT.0)WRITE(LINC)(IFT1(N),(PFT1(L,N),L=1,NF1R),N=1,NFT1);

        IF(NEXC.GT.0)WRITE(LINC)(IEXC(N),PEXC(N),N=1,NEXC);
	  ...
	  IF(NRNU.GT.0)WRITE(LINC)(IRNU(N),(RNU(M,N),M=1,MAXSP),N=1,NRNU);
     ...
     &	(IORD(N),(KORD(L,N),RORD(L,N),L=1,MAXORD),N=1,NORD);
	  ...
	  !=====Write PLOG Data to LINC `
	  !-Max.# of Pressure Point, and acture # of pressure point
	  WRITE(LINC) MAXNPPLOG,(NP_PLOG(K1),K1=1,II); 
        
	  !-The list for pressure points #
	  NM_PLOG(1:II)=0;
	  DO  K1=1,II
	    IF(NP_PLOG(K1)>0)THEN
	      K2=1;
	      DO K3=2,MAXNPPLOG
		    IF(ABS(P_PLOG(K1,K3)-P_PLOG(K1,1))<1.0E-30) K2=K2+1;
	      END DO
	      NM_PLOG(K1)=K2;
		END IF
	  END DO

	  WRITE(LINC)(NM_PLOG(K1),K1=1,II); 

	  DO K1=1,II
	    WRITE(LINC)(P_PLOG(K1,N),A_PLOG(K1,N),N_PLOG(K1,N),
     &                E_PLOG(K1,N),N=1,MAXNPPLOG);
	  END DO
      ELSE
        WRITE(LOUT,'(/A)')'WARNING...NO REACTION INPUT FOUND; ',
...
C
      ...
      DO 200 N=1,NSUB
         ...
         IF(I1.EQ.1)THEN
           KERR=.TRUE.;WRITE(LOUT,130) SUB(N)(:ILASCH(SUB(N)));
         ELSE
           IF(I1.LE.0)THEN;ISTR=SUB(N);ELSE;ISTR=SUB(N)(:I1-1);ENDIF
           ...
           ELSE
             IF(NN.LT.NDIM)THEN
               IF(ISTR(ILEN+1:) .NE. ' ') THEN
                 WRITE(LOUT,120) SUB(N)(:ILASCH(SUB(N)));KERR=.TRUE.;
               ELSE
                 NN=NN+1;STRAY(NN)=' ';STRAY(NN)=ISTR(:ILEN);
                 IF(I1.GT.0)THEN
                   I2=I1+INDEX(SUB(N)(I1+1:),'/');
				 ISTR=' ';ISTR=SUB(N)(I1+1:I2-1);
                   CALL IPPARR(ISTR,1,1,PAR,NVAL,IER,LOUT);
                   KERR=KERR.OR.(IER.NE.0);RAY(NN)=PAR(1);
                 ENDIF
               ENDIF
             ELSE
               WRITE(LOUT,110)SUB(N)(:ILASCH(SUB(N)));KERR=.TRUE.;
             ENDIF
           ENDIF
        ENDIF
  200 CONTINUE
...
C 1900 FORMAT (I4,'. ',A,T51,E10.3,F7.3,F11.3)
 1900 FORMAT (I4,'. ', A, T53, 1PE9.2, 2X, 0PF5.1, 2X, F9.1)
 1920 FORMAT (6X,A)
      RETURN
      END
C----------------------------------------------------------------------C
...
C
     ...
     &        LFORD,LRORD, LEIM, LJAN, LFT1, LEXC
      ...
      !LTHB = (NTHB.GT.0 .AND. ITHB(NTHB).EQ.II); Changed by GXL @20100830
      LTHB=.FALSE.;
	IF(NTHB.GT.0)THEN;IF(ITHB(NTHB).EQ.II)LTHB=.TRUE.;END IF

      !LFAL = (NFAL.GT.0 .AND. IFAL(NFAL).EQ.II); Changed by GXL @20100830
      LFAL=.FALSE.;
	IF(NFAL.GT.0)THEN;IF(IFAL(NFAL).EQ.II)LFAL=.TRUE.;END IF

      !LWL  = (NWL .GT.0 .AND. IWL(NWL).EQ.II); Changed by GXL @20100830
      LWL=.FALSE.;
	IF(NWL.GT.0)THEN;IF(IWL(NWL).EQ.II)LWL=.TRUE.;END IF
      	
	!LREV=(NREV.GT.0 .AND. IREV(NREV).EQ.II); Changed by GXL @20100830
	LREV=.FALSE.;
	IF(NREV.GT.0)THEN;IF(IREV(NREV).EQ.II)LREV=.TRUE.;END IF
      
	!LLAN=(NLAN.GT.0 .AND. ILAN(NLAN).EQ.II); Changed by GXL @20100830
	LLAN=.FALSE.;
	IF(NLAN.GT.0)THEN;IF(ILAN(NLAN).EQ.II)LLAN=.TRUE.;END IF

      !LRLT=(NRLT.GT.0 .AND. IRLT(NRLT).EQ.II); Changed by GXL @20100830
      LRLT=.FALSE.;
	IF(NRLT.GT.0)THEN;IF(IRLT(NRLT).EQ.II)LRLT=.TRUE.;END IF
      
	!LTRO=(NFAL.GT.0 .AND. IFAL(NFAL).EQ.II .AND. IFOP(NFAL).GT.2); Changed by GXL @20100830
	LTRO=.FALSE.;
	IF(NFAL.GT.0)THEN
	  IF(IFAL(NFAL).EQ.II.AND.IFOP(NFAL).GT.2)LTRO=.TRUE.;
	END IF

      !LSRI=(NFAL.GT.0 .AND. IFAL(NFAL).EQ.II .AND. IFOP(NFAL).EQ.2); Changed by GXL @20100830
      LSRI=.FALSE.;
	IF(NFAL.GT.0)THEN
	  IF(IFAL(NFAL).EQ.II.AND.IFOP(NFAL).EQ.2)LSRI=.TRUE.;
	END IF

      !LEIM=(NEIM.GT.0 .AND. IEIM(NEIM).EQ.II); Changed by GXL @20100830
      LEIM=.FALSE.;
	IF(NEIM.GT.0)THEN;IF(IEIM(NEIM).EQ.II)LEIM=.TRUE.;END IF

      !LJAN=(NJAN.GT.0 .AND. IJAN(NJAN).EQ.II); Changed by GXL @20100830
      LJAN=.FALSE.;
	IF(NJAN.GT.0)THEN;IF(IJAN(NJAN).EQ.II)LJAN=.TRUE.;END IF

      !LFT1=(NFT1.GT.0 .AND. IFT1(NFT1).EQ.II); Changed by GXL @20100830
      LFT1=.FALSE.;
	IF(NFT1.GT.0)THEN;IF(IFT1(NFT1).EQ.II)LFT1=.TRUE.;END IF

      !LEXC=(NEXC.GT.0 .AND. IEXC(NEXC).EQ.II); Changed by GXL @20100830
      LEXC=.FALSE.;
	IF(NEXC.GT.0)THEN;IF(IEXC(NEXC).EQ.II)LEXC=.TRUE.;END IF
C
      DO 500 N = 1, NSUB
         ILEN = ILASCH(SUB(N))
         KEY = ' '
...
C 1900 FORMAT (6X,A,T51,E10.3,F7.3,F11.3)
 1900 FORMAT (6X, A, T53, 1PE9.2, 2X, 0PF5.1, 2X, F9.1)
...
C
  ...
  150          CONTINUE
      ...
      !IF(NFAL.GT.0 .AND. IFAL(NFAL).EQ.II .AND. IFOP(NFAL).LT.0) THEN Zhanged by GXL @20100830
      IF(NFAL.GT.0)THEN
	  IF(IFAL(NFAL).EQ.II.AND.IFOP(NFAL).LT.0)THEN
          KERR=.TRUE.; WRITE(LOUT,1020);
	  END IF
      ENDIF

      !LREV=(NREV.GT.0 .AND. IREV(NREV).EQ.II); Changed by GXL @2010830
      LREV=.FALSE.;
	IF(NREV.GT.0)THEN
	  IF(IREV(NREV).EQ.II) LREV=.TRUE.;
	END IF

      !LLAN=(NLAN.GT.0 .AND. ILAN(NLAN).EQ.II); Changed by GXL @2010830
	LLAN=.FALSE.;
	IF(NLAN.GT.0)THEN
	  IF(ILAN(NLAN).EQ.II)LLAN=.TRUE.;
      END IF
	      
	!LRLT=(NRLT.GT.0 .AND. IRLT(NRLT).EQ.II) ; Changed by GXL @2010830
      LRLT=.FALSE.;
	IF(NRLT.GT.0)THEN
	  IF(IRLT(NRLT).EQ.II)LRLT=.TRUE.;
	END IF
...
C
	...
	!---------------------------------------------------------------
	! Get the MM,KK and II value to define the work space
	!
	!---------------------------------------------------------------
	SUBROUTINE GETSPACE_GOU(MM,KK,II)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IMPLICIT INTEGER(I-N)
	PARAMETER (LIN=15,LOUT=16)
	DIMENSION VALUE(5)
	CHARACTER LINE*80,SUB(80)*80,KEY(5)*4,AUNITS*4,EUNITS*4,UPCASE*4,
     &	      IUNITS*80;
      LOGICAL KERR
  ...
  105 CONTINUE
	...
	IF(ILEN.EQ.0) GOTO 100;
      ...
      IF(NKEY.EQ.1.OR.NKEY.EQ.2)THEN !ELEMENT OR SPECIES DATA
        ITASK=NKEY;IF(NSUB.EQ.1) GOTO 100;
        DO N=2,NSUB;SUB(N-1)=' ';SUB(N-1)=SUB(N);END DO
        NSUB=NSUB-1;
      ELSEIF(NKEY.EQ.4)THEN          !START OF REACTIONS; ARE UNITS SPECIFIED?
        ITASK=4;
        CALL CKUNIT(LINE(:ILEN),AUNITS,EUNITS,IUNITS);
        GOTO 100
      ENDIF
      ...
      !====功象ITSAK序佩斤哘侃尖   
      IF(ITASK.EQ.1)THEN !ELEMENT DATA
        !IF(NSUB.GT.0)CALL CKCHAR(SUB,NSUB,MDIM,ENAME,AWT,MM,KERR,LOUT); !資函欺Element方楚才斤哘議圷殆
	   IF(NSUB.GT.0)CALL CKCHAR_GOU(SUB,NSUB,MM,KERR,LOUT); !資函欺Element方楚才斤哘議圷殆
      ELSEIF(ITASK.EQ.2)THEN !PROCESS SPECIES DATA
        !IF(NSUB.GT.0)CALL CKCHAR(SUB,NSUB,KDIM,KNAME,WTM,KK,KERR,LOUT);!SPECIES 
        IF(NSUB.GT.0)CALL CKCHAR_GOU(SUB,NSUB,KK,KERR,LOUT);!SPECIES 
      ELSEIF (ITASK.EQ.4) THEN !PROCESS REACTION DATA
        IND=0;
        DO N=1,NSUB
          IND=MAX(IND,INDEX(SUB(N),'/'));
	  ...
	  IF(IND.LE.0) II=II+1; !柴麻Reaction方楚
      ENDIF
      GOTO 100
...
!----------------------------------------------------------------------
	SUBROUTINE CKCHAR_GOU(SUB,NSUB,NN,KERR,LOUT)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      IMPLICIT INTEGER(I-N)
      CHARACTER*(*)SUB(*);
      CHARACTER ISTR*80,UPCASE*4;
