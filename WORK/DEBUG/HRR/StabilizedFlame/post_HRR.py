...
def graph_HRR(save_dir, ev_num=1):
    ...
    for j, df in data_nout_dict.items():
        ...
        df.plot(ax=ax, x='xcm', y='HRR', ls='--', lw=2, marker=mk, c='r', alpha=0.8, legend=False, label='HRR (Enthalpy diff)')
        df.plot(ax=ax, x='xcm', y='HRD', ls='-.', lw=2, marker=mk, c='y', alpha=0.8, legend=False, label='HRR (Omega dot)')
        df.plot(ax=ax, x='xcm', y='HRRct', ls='-', lw=2, marker=mk, c='c', alpha=0.8, legend=False, label='HRR (Cantera postprocessing)', zorder=-1)

        ax.set_ylabel('HRR [J/m$^3$-s]')
        ax.set_xlabel('r [cm]')
        ...
        if flag_log:
            ax.set_ylim(HRRlim_log)
            ax.set_yscale('log')

        ax.set_title('t = {:1.5f} s'.format(float(attr_nout_dict[j]['time from 0s'])), fontsize=18)
...
if __name__ == "__main__":
    ...
    save_dir_sub = save_dir_sub+'_HRR'
    ...
    for i in num_list:
        ...
        graph_HRR(save_dir, ev_num=every_num_graph)
    mkmovie(save_dir)
