...
C@(#)  ---------------  VERSION = 1.23
C@(#)  |  SCCS  FILE |
C@(#)  |   SUMMARY   |  CURRENT CHECKOUT DATE = 10/03/94
C@(#)  ---------------                           at 16:09:54
C@(#)                   DATE OF NEWEST DELTA = 10/03/94
C@(#)                                            at 16:09:54
C@(#)  SCCS file name = /users/chemkin/SCCS/s.tranlib.f
C@(#)===================================================================
...
C
   ...
   15 FORMAT(
     ...
     2/'           CHEMKIN-II Version 3.8, January 1995',
C*****precision > double
     3/'           DOUBLE PRECISION')
C*****END precision > double
C*****precision > single
...
C*****precision > double
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
      DOUBLE PRECISION ZERO,        ONE,       EIGHT
      PARAMETER        (ZERO=0.0D0, ONE=1.0D0, EIGHT=8.0D0)
C*****END precision > double
C
C*****precision > single
C      IMPLICIT REAL (A-H, O-Z), INTEGER (I-N)
C      REAL              ZERO,        ONE,       EIGHT
...
C    THIS SUBROUTINE USES HORNERS ALGORITHM TO EVALUATE A POLYNOMIAL
C    FIT.  THIS ROUTINE IS NOT NORMALLY CALLED BY THE PACKAGE USER.
C    The polynomial fit is hard coded for order four polynomials,
C    NO = 4.
C
C  INPUT-
C    TF      - INDEPENDENT VARIABLE OF FIT. EITHER TEMPERATURE
