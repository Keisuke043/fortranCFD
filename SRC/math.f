...
C
C  $Header: /chemkin/4.2/prebuild/source_public/math.f 20    3/08/07 5:04p Cwang $
C
C  $NoKeywords: $
C***********************************************************************END-HDR
C
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           DGER, DSCAL, DSWPCK, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
            IF( JP.NE.J )
     $         CALL DSWPCK( N, A( J, 1 ), LDA, A( JP, 1 ), LDA )
*
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           DGEMM, DGEMV, DSWPCK, DTRSM, DTRTRI, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      DO 60 J = N - 1, 1, -1
         JP = IPIV( J )
         IF( JP.NE.J )
     $      CALL DSWPCK( N, A( 1, J ), 1, A( 1, JP ), 1 )
   60 CONTINUE
*
      WORK( 1 ) = IWS
      RETURN
     ...
     $   RETURN
      ...
      IF( INCX.EQ.1 ) THEN
         DO 10 I = K1, K2
            IP = IPIV( I )
            IF( IP.NE.I )
     $         CALL DSWPCK( N, A( I, 1 ), LDA, A( IP, 1 ), LDA )
   10    CONTINUE
...
c
  ...
  120       continue
            l = ipvt(k)
            if (l .ne. k) call dswpck(n,a(1,k),1,a(1,l),1)
  130    continue
  140    continue
  150 continue
      return
...
c                = 0  normal value.
c                = k  if  u(k,k) .eq. 0.0 .  this is not an error
c                     condition for this subroutine, but it does1
...
c
   ...
   50 continue
      ...
      subroutine  dswpck (n,dx,incx,dy,incy)
c
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SGER, SSCAL, SSWPCK, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
            IF( JP.NE.J )
     $         CALL SSWPCK( N, A( J, 1 ), LDA, A( JP, 1 ), LDA )
*
*           Compute elements J+1:M of J-th column.
*
            IF( J.LT.M )
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SGEMM, SGEMV, SSWPCK, STRSM, STRTRI, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      DO 60 J = N - 1, 1, -1
         JP = IPIV( J )
         IF( JP.NE.J )
     $      CALL SSWPCK( N, A( 1, J ), 1, A( 1, JP ), 1 )
   60 CONTINUE
*
      WORK( 1 ) = IWS
      RETURN
     ...
     $   RETURN
      ...
      IF( INCX.EQ.1 ) THEN
         DO 10 I = K1, K2
            IP = IPIV( I )
            IF( IP.NE.I )
     $         CALL SSWPCK( N, A( I, 1 ), LDA, A( IP, 1 ), LDA )
   10    CONTINUE
      ELSE IF( INCX.GT.1 ) THEN
         DO 20 I = K1, K2
            IP = IPIV( IX )
...
c
  ...
  120       continue
            l = ipvt(k)
            if (l .ne. k) call sswpck(n,a(1,k),1,a(1,l),1)
  130    continue
  140    continue
  150 continue
      return
...
c
   ...
   50 continue
      ...
      subroutine sswpck (n,sx,incx,sy,incy)
c
c     interchanges two vectors.
c     uses unrolled loops for increments equal to 1.
c     jack dongarra, linpack, 3/11/78.
...
C*****EXCLUDE-IF precision_single
      DOUBLE PRECISION
C*****END EXCLUDE-IF precision_single
...
C=======================================================================
C The following codes are taken from NETLIB distribution of EISPACK,
C     http://www.netlib.no/netlib/eispack/index.html
C by Burton S. Garbow, Mathematics and Computer Science Division,
C Argonne National Laboratory, August 1983.
C=======================================================================
