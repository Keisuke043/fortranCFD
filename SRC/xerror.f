...
C
C  $Header: /chemkin/4.1/prebuild/source_public/xerror.f 18    12/23/05 9:59a Frupley $
C
C  $NoKeywords: $
C***********************************************************************END-HDR
C
...
C**End
      ...
      INTEGER IPAR, IVALUE
C-----------------------------------------------------------------------
      INTEGER LUNIT, LUNDEF, MESFLG, JUNK, LUNSAV, MFLGSV, J4SAVE
      EXTERNAL LUNSAV, MFLGSV, J4SAVE
C-----------------------------------------------------------------------
C The following Fortran-77 declaration is to cause the values of the
...
C***FIRST EXECUTABLE STATEMENT  IXSAV
      IF (IPAR .EQ. 1) THEN
        ...
        IF (ISET) THEN
           LUNIT = IVALUE
           JUNK = LUNSAV(LUNIT,ISET)
           JUNK = J4SAVE(3,LUNIT,ISET)
        ENDIF
      ENDIF
C
      IF (IPAR .EQ. 2) THEN
